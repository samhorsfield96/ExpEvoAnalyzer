...
rule all:
   input:
        ...
        expand(f"{config['output_dir']}/{{sample}}.vcf", sample=SAMPLES)
...
checkpoint ska_build:
    ...
    ska build -o {output.outpref} -k {params.ksize} --threads {threads} -f {input.infile} > {log}
    """
...
def get_ska_build(wildcards):
    ...
    return f"{config['output_dir']}/ska_{wildcards.sample}.skf"
...
rule ska_map:
    ...
    shell:
        """
        ska map -f vcf --threads {threads} {input.contigs} {input.infile} > {output.outfile}
        """
